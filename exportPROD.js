...
const fs = require('fs');
const serviceAccount = require("./_credentials/firebase.prod.json");
...
admin.initializeApp({
  ...
  databaseURL: "https://lukb-events.firebaseio.com"
});
...
results.then(dt => {  
  getSubCollection(dt).then(() => {    
    ...
    fs.writeFile(fileName, JSON.stringify(data, null, 4), 'utf8' , function(err) {
        ...
        console.log("Collection '"+collectionName+"' successfully exported from PROD to file: " + fileName);
    });
  })  
})
